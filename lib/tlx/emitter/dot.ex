...
defmodule TLX.Emitter.Dot do
  ...
  def emit(module, opts \\ []) do
    ...
    render(module_name, states, edges, initial)
  end

  defp detect_state_var(variables, actions) do
    # Find a variable where all transition values are atoms
    all_transitions =
    ...
    # Prefer the variable that appears most in transitions with atom values
    candidates
    |> Enum.max_by(
      fn name ->
  ...
  defp extract_guard_state({:expr, ast}, state_var) do
    extract_guard_states_from_ast(ast, state_var)
    |> case do
      [] -> nil
      states -> states
    end
  ...
  # Recurse into `and` — collect from both sides
  ...
  defp render(name, states, edges, initial) do
    state_nodes =
      states
      |> MapSet.to_list()
      |> Enum.sort()
      |> Enum.map(fn state ->
        shape = if state == initial, do: "doublecircle", else: "circle"
        "  #{state} [shape=#{shape}]"
      end)
    ...
    # Edges from :_any (no guard on state var) — draw from all states
    any_edges =
      edges
      |> Enum.filter(fn {src, _, _} -> src == :_any end)
      |> Enum.flat_map(fn {:_any, tgt, label} ->
        Enum.map(MapSet.to_list(states), fn src ->
